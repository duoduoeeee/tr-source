...
现在，从主屏幕下拉仍然显示的是spotlight搜索栏（以及建议的应用程序）。而搜索栏还存在于你主屏左边专门的搜索页（见动图）。而这个专门的搜索页中还包含了Siri建议的应用和联系人，而它还和地图功能做结合来显示当地的兴趣热点（nearby）。有些推荐颇为不着边际——但是我还是频繁地使用“加油站位置”功能——比我所愿意承认的次数多得多。（在nearby）下面你则会看见从苹果新闻应用直接拉取的消息。呃但是有时候苹果新闻并不能显示。。呃但无论如何大多数的时候还是能行的。。有时候它不能显示，我也不知道原因是啥。当然，如果我刚刚从苹果新闻应用里退出来，再进搜索还出这些新闻倒也没什么意义了，但是应该不是这原因。。走着看吧。
...
如果要说ios9的哪个部分还有那种半成品的感觉的话，我想应该是苹果新出的[新闻应用](http://www.engadget.com/2015/06/08/apple-news-app/)了。新闻应用的出现宣示着我们之前熟知的newsstand的终结。而newsstand我们多数人都不会注意到，它是那种被我们扔到一个从不去碰的文件夹之后为我们所遗忘的应用，但这次，苹果新闻是一个颜值不错的RSS阅读器，有着很实用的特性——它还是有些潜力，但也还没算达到。首次进入应用时，你得选出你喜欢的新闻源和喜欢的新闻主题，然后你就会被跳转到一个列表（在iPhone上）或者一个图片网格里（在iPad上）。新闻则显示在里面供你查看。接下来很快你就会发现新闻应用在处理不同文件之间的差异——大多数新闻都只是在白色背景上排好字，外加顶上一个新闻源的图标，而有几家出版商受到了苹果的特别关注，它们的新闻在视觉上更为华丽
